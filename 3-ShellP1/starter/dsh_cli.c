...
int main()
{
    ...
    while (1)
    {
        ...
        if (strcmp(cmd_buff, "dragon") != 0)
        {
            ...
            if (rc == OK){
                ...
                for (int commandCount = 0; commandCount <= clist->num; commandCount++)
                {
                    ...
                    printf("<%d> %s ", commandCount + 1, clist->commands[commandCount].exe);
                    if (strcmp(clist->commands[commandCount].args, blankString) != 0)
                    {
                        printf("[%s]\n", clist->commands[commandCount].args);
                    }
                    else printf("\n");
        ...
        for (int commandCount = 0; commandCount <= clist->num; commandCount++)
        {
            strcpy(clist->commands[commandCount].exe, "                   ");
            strcpy(clist->commands[commandCount].args, "                   ");
        }
    }
